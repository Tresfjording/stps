...
def build_weekly_filename(
    ...
    return f"{base_name}_uke_{report_week}_{report_year}{extension}"
...
report_html_filename = build_weekly_filename("report_", ".html", report_week, report_year)
report_pdf_filename = build_weekly_filename("report_", ".pdf", report_week, report_year)
...
if source_workbook is not None:
    ...
    if weekly_df is not None and weekly_total_col is not None and len(weekly_df) > 0:
        ...
        # Workbook contains weekly data in table Summasummarium5 even when named range is broken.
        try:
